...
abstract class Application {
	...
	final public function __construct() {
		$this->_eventsHandler = new \salodev\EventsHandler();
	}
	 ...
	 * @return \Window
	 */
    public function openWindow($className = '\cuif\Window', array $params = array()) {
	...
	public function confirmWindow($text, $onConfirm = null, $onCancel = null) {
		...
		if ($onConfirm !== null) {
			$window->bind('confirm', $onConfirm);
		}
		if ($onCancel !== null) {
			$window->bind('cancel', $onCancel);
		}
		return $window;
	}
	
	public function promptWindow($title, $text, $value, callable $onConfirm = null, callable $onCancel = null) {
		$window = $this->openWindow('\cuif\PromptWindow', array(
			'title' => $title,
			'text'  => $text,
			'value' => $value,
		));
